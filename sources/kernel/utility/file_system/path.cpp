// #include "path.hpp"

// #include <algorithm>

// #include "kernel/framework/core/kernel.hpp"

// #include "path_storage.hpp"
// #include "path_values.hpp"

// std::string
// utils::Path::getName(std::string_view a_path) noexcept
// {
//     std::string result;
//     result.reserve(a_path.size());
//     int start = a_path.size() - 1;
//     if (a_path[start] == PATH_SEPARATOR)
//     {
//         --start;
//     }
//     for (int i = start; i >= 0; --i)
//     {
//         if (a_path[i] != PATH_SEPARATOR)
//         {
//             result.push_back(a_path[i]);
//         }
//         else
//         {
//             break;
//         }
//     }
//     if (result.empty())
//     {
//         LOG_ERROR("There is no file or folder at the end of the path: '%s'",
//                   a_path);
//     }
//     else
//     {
//         std::reverse(result.begin(), result.end());
//     }
//     return result;
// }

// std::string
// utils::Path::normalizeFolderPath(std::string_view a_path,
//                                 bool sep_postfix) noexcept
// {
//     std::string result(a_path);
//     if (!result.empty())
//     {
//         if (sep_postfix)
//         {
//             if (result.back() != PATH_SEPARATOR)
//             {
//                 result.push_back(PATH_SEPARATOR);
//             }
//         }
//         else
//         {
//             if (result.back() == PATH_SEPARATOR)
//             {
//                 result.pop_back();
//             }
//         }
//     }

//     return result;
// }

// std::string
// utils::Path::getRelativeToApp(std::string_view a_path, bool is_folder)
// noexcept
// {
//     std::string result;
//     auto& app_path = KERNEL.getFolderPath(APP_PATH_NAME);
//     result         = combine(is_folder, app_path, a_path);
//     return result;
// }
