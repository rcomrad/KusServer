...
{
    auto log = dom::VariableStorage::touchWord(str::string("log_type"));
...
void
gen::Log::setLogType(const str::string& aLogType) noexcept
{
    static std::unordered_map<str::string, PrintStatus> statusMap = {
...
void
gen::Log::setOutputType(const str::string& aOutputType) noexcept
{
    clear();
    if (aOutputType == "cout")
    {
...
//         // Copy the error message into a str::string.
